...
{ 
	...
	frag_colour = texture(texture_diffuse1, texcoord) * texture(texture_occlusion1, texcoord) * ((ambient_color) + vec4(diffuse, 1.0f) + vec4(directional, 1.0f) + vec4(specular, 1.0f)); 
}
